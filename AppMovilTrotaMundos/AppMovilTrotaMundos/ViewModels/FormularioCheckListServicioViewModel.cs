...
{
    public class FormularioChecklistServicioViewModel : BaseViewModel
    {

        private readonly ApiService _apiService;
        private Models.CheckListServicio _checklist;

        private Stopwatch _stopwatch;  // Cronómetro
        private string _tiempoTranscurrido;

        public Command GuardarCommand { get; }
        public Command ModificarCheckServicioCommand { get; }
        ...
        public Models.CheckListServicio CheckList
        {
            get => _checkList;
            set
            {
                _checkList = value;
                OnPropertyChanged(nameof(CheckList)); // Notificar que CheckList ha cambiado
                ActualizarNumeroSerie(); // Actualizar el número de serie cuando el CheckList cambia
            }
        }
        ...
        // Dictionaries for form data

        public ICommand SeleccionarSemaforoCommand { get; }
        ...
        public FormularioChecklistServicioViewModel()
        {
            //SeleccionarSemaforoCommand = new Command<string>(OnSeleccionarSemaforo);

            _apiService = new ApiService();
            CheckList = new Models.CheckListServicio();
            ...
            ModificarCheckServicioCommand = new Command(async () => await OnModificarCheck());
        ...
        private async Task GuardarChecklistAsync()
        {
            try
            {
                ...
                await _apiService.AddServicioAsync(CheckList);
        ...
        private async Task OnModificarCheck()
        {
            try
            {
                ...
                var numeroserie = CheckList.NumeroSerie;
                ...
                bool isUpdated = await _apiService.UpdateCheckServicioAsync(CheckList);
        ...
        {
            // Asegurarte de que CheckList no sea null antes de usarlo
            if (CheckList != null)
            {
                NumeroSerie = CheckList.NumeroSerie;
                Debug.WriteLine($"NumeroSerie actualizado: {NumeroSerie}");
            }
        }
        ...
        //private void OnSeleccionarSemaforo(string parametro)
        //{
        //    // parametro será "Rojo,lectura_codigos_semaforo"
        //    var partes = parametro.Split(',');
        //    if (partes.Length == 2 && partes[1] == "lectura_codigos_semaforo")
        //    {
        //        CheckList.lectura_codigos_semaforo = partes[0];
        //        OnPropertyChanged(nameof(CheckList));
        //    }
        //}
